 ...
 *
 * HM-10 BLE: TX→RX(10), RX→TX(11) — 9600 baud
 *
...
SoftwareSerial ble(10, 11);  // RX = 10, TX = 11
...
// ── Setup ─────────────────────────────────────────────────────────
void setup() {
  Serial.begin(9600);           // For debugging on USB
  ble.begin(9600);              // HM-10 default is usually 9600 — change to 115200 if needed
...
void loop() {
  ...
  // 2. Smooth speed ramp (runs every ~40 ms)
...
//void setup() {
//
//  Serial.begin(9600);   // For the Computer Monitor
//  ble.begin(9600);
//
//  pwm.begin();
//  pwm.setPWMFreq(60);
